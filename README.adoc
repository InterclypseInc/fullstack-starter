...
Running `sdk list java` will have sdkman list out the available versions. This application requires Java 17, so select a distribution that is based on Java 17. The exact patch versions on offer change over time, so check `sdk list java` for the current ones.

For example:

    sdk install java 17.0.13-tem

will install version 17.0.13 provided by Eclipse Temurin. Alternatively you could select

    sdk install java 17.0.13-ms

to install 17.0.13 from Microsoft.

Make sure the Java 17 install is the *active* one (`sdk use java 17.0.13-tem`, or `sdk default java 17.0.13-tem` to make it permanent) before running any gradle commands. Gradle itself has to run on Java 17 — Spring Boot's gradle plugin is compiled for 17, so an older JDK fails at configuration time with `No matching variant of org.springframework.boot:spring-boot-gradle-plugin ... compatible with Java 11`.
